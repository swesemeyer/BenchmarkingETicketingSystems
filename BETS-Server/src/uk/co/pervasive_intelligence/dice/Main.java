...
import uk.co.pervasive_intelligence.dice.protocol.control.setup.Setup;
...
public class Main {
  ...
  @Parameter(
      names = { "--client-output", "-c" },
      description = "Optionally output the client protocol timings to a CSV file")
  ...
  @Parameter(
      names = { "--input-dh", "-i" },
      description = "Load DH parameters from file")
  ...
  @Parameter(
      names = { "--log-level", "-l" },
      description = "Level of logging: 0 off, 1 error, 2, warn, 3 info, 4 debug, 5 trace, 6 all")
  private int                 logLevel           = LOG_LEVEL_DEFAULT;

  /** Protocol run command line parameters. */
  @Parameter(
      names = { "--output-dh", "-o" },
      description = "Save DH parameters to file")
  private String              outputDH           = null;

  /** Protocol run command line parameters. */
  @Parameter(
      names = { "--run", "-r" },
      description = "Run a protocol (:iterations:parameter1:parameter2:...)")
  private ProtocolRun         protocolRun        = null;

  /** The name of the server timing output file, if any. */
  @Parameter(
      names = { "--server-output", "-s" },
      description = "Optionally output the server protocol timings to a CSV file")
  private String              serverOutput       = null;

  /** The name of the server setup timing output file, if any. */
  @Parameter(
      names = { "--setup-output", "-u" },
      description = "Optionally output the server setup timings to a CSV file")
  private String              setupOutput        = null;

  /** The name of the server tear down timing output file, if any. */
  @Parameter(
      names = { "--tear-down-output", "-d" },
      description = "Optionally output the server tear down timings to a CSV file")
  private String              tearDownOutput     = null;

  /** Flag to indicate that the protocol should use Diffie-Hellman. */
  @Parameter(
      names = { "--use-dh", "-e" },
      description = "Use DH parameters")
  ...
  public Main() {
    ...
    // These are needed only because Eclipse's clean up will try to make the fields final because they are set by JCommander, and
    // not explicitly.
   ...
   *
   * @param filename The file to load the DH parameters from.
  ...
  /**
   * Outputs the timing CSV file header and returns the associated cell processors.
   *
   * @param csvWriter The CSV writer for the file.
   * @param names The names of the fields for the header.
  ...
  private CellProcessor[] outputCSVHeader(ICsvListWriter csvWriter, final List<String> names) throws IOException {
    ...
    // Add in three fields per timing: the time, the count and the number of bytes processed
   ...
   *
   * @param csvWriter The CSV writer for the file.
   * @param names The names of the fields to get the order correct.
   * @param cellProcessors The cell processors used to output the CSV fields.
   * @param iteration The current iteration number.
   * @param timings The timings to be output.
   * @throws IOException If there was a problem writing the timings.
   */
  private void outputCSVTimings(ICsvListWriter csvWriter, final List<String> names, CellProcessor[] cellProcessors, int iteration,
      final Map<String, Timing> timings) throws IOException {
   ...
   * @param dhParameters The DH parameters to output.
   */
   ...
   * 
   * @param timings the list of timings to be output
  ...
  private void outputTimings(List<Map<String, Timing>> timings, String filename) {
    ...
    if (timings.size() > 0) {
      try {
        ...
        // Get the list of names in sorted order for consistency. We hope they match for each protocol run.
  ...
  private void run() {
    ...
    final ServerData serverData = new ServerData(crypto.getPublicKey().getEncoded(), this.logLevel, this.protocolRun,
        this.keyLength, crypto.getDhParameters());

    // Warm up the encryption so that loading of the encryption provider does not slow things down.
    ...
    if (clazz != null) {
      ...
      try {
        for (int i = 1; i <= this.protocolRun.getIteration(); i++) {
          ...
          // Sometimes setup just fails to get through to the Android device. Try running for a fixed number of times until success.
          if (!setup.run()) {
