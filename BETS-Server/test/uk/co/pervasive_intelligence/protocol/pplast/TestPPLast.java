 ...
 *
 */
package uk.co.pervasive_intelligence.protocol.pplast;
...
public class TestPPLast {
  ...
  public void setUp() throws Exception {
    ...
    LOG.debug("Hash2: " +sharedMemory.Hash2+" : "+ base64.encodeToString(crypto.getHash(testHash, sharedMemory.Hash2)));
    LOG.debug("Hash3: " +sharedMemory.Hash3+" : "+ base64.encodeToString(crypto.getHash(testHash, sharedMemory.Hash3)));
  ...
  public void testProtocol() {
    ...
    for (int i = 0; i < Actor.VERIFIER.length; i++) {
      ...
      time_start = Instant.now().toEpochMilli();
      overall_start = time_start;
      sharedMemory.actAs(Actor.VERIFIER[i]);
      data = this.generateVerifierIdentity(Actor.VERIFIER[i]);
      time_end = Instant.now().toEpochMilli();
      durationInMS = time_end - time_start;
      ...
      time_start = Instant.now().toEpochMilli();
      sharedMemory.actAs(Actor.VERIFIER[i]);
      success = this.verifyVerifierCredentials(Actor.VERIFIER[i], data);
      time_end = Instant.now().toEpochMilli();
      durationInMS = time_end - time_start;
    ...
    time_start = Instant.now().toEpochMilli();
    String verifierID = Actor.VERIFIER[1];
  ...
  private byte[] generateTicketDetails(byte[] data) {
    ...
    if (numberOfVerifiers != evenNumberOfVerifiers) {
      ...
      final String ID_du = Actor.VERIFIER[Actor.dummyVerifierIndx];
    ...
    sendDataList.add(BigInteger.valueOf(evenNumberOfVerifiers).toByteArray()); // need to keep track of the array size
